...
let settings = { arbiter: { enabled: true, timeoutMs: 1600, toastResults: false, profileId: 'p1', seedTranscriptK: 400, seedMemoryK: 120, seedOutTokens: 4000 } };
...
(async () => {
  ...
  const cap = Math.min(2000, Math.max(4, settings.arbiter.seedTranscriptK)) * 1000;
  ok('transcript cap scales with setting (400k)', cap === 400000);
  const outClamp = Math.min(8000, Math.max(400, settings.arbiter.seedOutTokens));
  ok('output tokens honor setting (4000)', outClamp === 4000);
  const memCap = Math.min(500, Math.max(2, settings.arbiter.seedMemoryK)) * 1000;
  ok('memory cap honors setting (120k)', memCap === 120000);
